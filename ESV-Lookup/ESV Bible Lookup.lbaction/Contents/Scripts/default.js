  ...
  if (apiToken == undefined) {
    ...
    var result = HTTP.getJSON(
      'https://api.esv.org/v3/passage/text/?q=' +
        encodeURI(argument) +
        '&include-passage-references=false&include-verse-numbers=false&include-footnotes=false&include-headings=false&include-short-copyright=false',
      {
        headerFields: {
    ...
    if (LaunchBar.options.shiftKey) {
      ...
      // Paste
      LaunchBar.paste(passage + ' (' + argument + ' ESV)');
    }
  }
}
...
function setApiKey() {
  var response = LaunchBar.alert(
    'API key required',
    '1) To obtain an API key, you will first need to  create an API Application.\n2) When done copy the API key.\n3) Press »Set API key«',
    'Obtain API key',
    'Set API key',
    'Cancel'
  );
    ...
    case 1:
      ...
      if (clipboardConent.length == 40) {
        ...
        if (test.response.status != 403) {
          ...
          LaunchBar.alert(
            'Success!',
            'API key set to: ' + Action.preferences.apiToken + '.'
          );
        } else {
