...
function run(folder) {
  ...
  try {
    var plainTextFileTypes = File.readPlist(
      '/Applications/LaunchBar.app/Contents/Resources/PlainTextFileTypes.plist'
    );
  } catch (exception) {
  ...
  try {
    var contentTypePlist = File.readPlist(
  ...
  var typeDeclarationsSystem = UTImportedTypeDeclarations.concat(
  ...
  for (var i = 0; i < apps.length; i++) {
    if (apps[i].endsWith('.app')) {
      ...
      if (File.exists(appInfoPlist)) {
        ...
        if (
          content.UTImportedTypeDeclarations != undefined &&
          content.UTExportedTypeDeclarations != undefined
        ) {
          ...
          var typeDeclarationsApp = UTImportedTypeDeclarations.concat(
            UTExportedTypeDeclarations
          );
        } else if (
          content.UTImportedTypeDeclarations != undefined &&
  ...
  presentationExtensions = presentationExtensions.toString().split(',');
  ...
  imageExtensions = imageExtensions
    ...
    .split(',');
  movieExtensions = movieExtensions.toString().split(',');
  audioExtensions = audioExtensions.toString().split(',');

  // presentations
  var presentationData = [];
  for (var i = 0; i < presentationExtensions.length; i++) {
    if (
      !presentationData.includes(presentationExtensions[i]) &&
      presentationExtensions[i] != ''
    ) {
  ...
  for (var i = 0; i < documentExtensions.length; i++) {
    if (
      !docData.includes(documentExtensions[i]) &&
      documentExtensions[i] != ''
    ) {
  ...
  // var contents = LaunchBar.execute('/bin/ls', '-tA', folder)
  //         .trim()
  //         .split('\n')

  // LaunchBar.alert(contents)
  ...
  for (var i = 0; i < contents.length; i++) {
    ...
    var hasExtension = item.match(/\.([\w]+$)/);

    if (hasExtension != null) {
      var extension = hasExtension[1].toLowerCase();
    } else {
      var extension = '';
    }
