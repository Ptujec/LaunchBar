...
// Constants
let what3wordsRegex = try! NSRegularExpression(
    pattern: "(?:[a-züäöß]+\\.){2}[a-züäöß]+"
)
let dividers = [
    " to ": ("to", "My Location"), " nach ": ("nach", "My Location"),
...
func handleInput(_ input: String) {
    ...
    for (divider, (action, myLoc)) in dividers {
        if input.contains(divider) {
            ...
            let saddr =
                isReversed
                ? (parts[1].isEmpty ? myLoc : parts[1])
                : (parts[0].isEmpty ? myLoc : parts[0])
            let daddr = isReversed ? parts[0] : parts[1]

            let saddrIcon =
                saddr.range(
                    of: what3wordsRegex.pattern,
                    options: .regularExpression
                ) != nil ? "w3wCircleTemplate" : "circleTemplate"
            let daddrIcon =
                daddr.range(
                    of: what3wordsRegex.pattern,
                    options: .regularExpression
                ) != nil ? "w3wPinTemplate" : "pinTemplate"
...
class SearchCompleterDelegate: NSObject, MKLocalSearchCompleterDelegate {
    func completerDidUpdateResults(_ completer: MKLocalSearchCompleter) {
        ...
        print(
            String(
                data: try! JSONSerialization.data(withJSONObject: suggestions),
                encoding: .utf8
            )!
        )
    }
...
func fetchWhat3WordsSuggestions(_ what3words: String) {
    var components = URLComponents(
        string: "https://mapapi.what3words.com/api/autosuggest"
    )!
    ...
    URLSession.shared.dataTask(with: URLRequest(url: components.url!)) {
        ...
        do {
            ...
            let suggestions = response.suggestions.map {
                [
                    "title": $0.words, "subtitle": $0.nearestPlace,
                    "icon": "w3wPinTemplate",
                ]
            }
            print(
