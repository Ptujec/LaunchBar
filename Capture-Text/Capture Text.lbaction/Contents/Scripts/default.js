...
function run() {
  if (File.exists('/usr/local/bin/ocr')) {
    var exec = '/usr/local/bin/ocr';
  } else if (File.exists('/opt/homebrew/bin/ocr')) {
    var exec = '/opt/homebrew/bin/ocr';
  }

  if (exec == undefined) {
    LaunchBar.alert('OCR could not be found!');
    return;
  }

  var result = LaunchBar.execute(exec).trim();
  ...
  if (LaunchBar.options.commandKey) {
    ...
    if (
      ...
      (/\.\w+/.test(result) && !result.includes('@'))
    ) {
      if (!result.includes('http')) {
        result = 'http://' + result;
      }
      var urls = result.match(
        /(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])|(www\.[\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?/gi
      );
      ...
      result = result.replace(/\(0\)/g, '').replace(/[^0-9\+]/g, '');
      var tel = 'tel://' + result;
